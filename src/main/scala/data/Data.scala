...
object Data {
  ...
  val functions: List[FunctionInfo] = {
    ...
    List(
      ...
      F("List", "tail", Nil, Nil, "List[A]", listImg("tail.svg"), List(dangerT("例外"), infoT("部分取得")), "先頭以外を取得.<br>Nilの場合は例外."),
      F("List", "tails", Nil, Nil, "Iterator[List[A]]", listImg("tails.svg"), List(infoT("部分取得")), "元のListからNilになるまで先頭要素を0~n個削る過程のList群の取得."),
      ...
      F("List", "filter", Nil, List(T("p", "A => Boolean")), "List[A]", listImg("filter.svg"), List(infoT("絞り込み")), "pがtrueになる要素を取得."),
      F("List", "filterNot", Nil, List(T("p", "A => Boolean")), "List[A]", listImg("filterNot.svg"), List(infoT("絞り込み")), "pがfalseになる要素を取得."),

      F("List", "distinct", Nil, Nil, "List[A]", listImg("distinct.svg"), List(infoT("絞り込み"), infoT("重複排除")), "重複を排除したListを取得.<br>重複分は先頭に近い方を維持."),
      F("List", "distinctBy", List("B"), List(T("f", "A => B")), "List[A]", listImg("distinctBy.svg"), List(infoT("絞り込み"), infoT("重複排除")), "fで変換した結果の重複を排除したListを取得.<br>重複分は先頭に近い方を維持."),
